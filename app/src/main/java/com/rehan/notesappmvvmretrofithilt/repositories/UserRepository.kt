...
class UserRepository @Inject constructor(private val userAPI: UserAPI) {
    ...
    val userResponseLiveData: LiveData<NetworkResult<UserResponse>>
        get() = _userResponseMutableLiveData       // We will pass the mutable livedata object to public livedata so that it can be observed by observers such as fragment, activity
    ...
    private fun handleResponse(response: Response<UserResponse>) {
        if (response.isSuccessful && response.body() != null) {
            ...
            _userResponseMutableLiveData.postValue(NetworkResult.Success(response.body()!!))   // Here we will set our API response in mutable live data object
            ...
            _userResponseMutableLiveData.postValue(NetworkResult.Error(errorObject.getString("message")))      // "message" is the key name in API in retrofit error body like message = User already exists
        } else {
